...
dataset_dir = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'dataset/coco/link2coco2017'))
...
tr_hdf5_path = os.path.join(dataset_dir, "coco_train_dataset480.h5")
val_hdf5_path = os.path.join(dataset_dir, "coco_val_dataset480.h5")

val_size = 1000  # size of validation set  设置的validation subset的大小.　剩余的val数据将选如train数据中
image_size = 368  # 用于训练网络时，设定的训练集图片的统一尺寸　
...
def make_mask(img_dir, img_id, img_anns, coco):
    ...
    # plt.show()
    # # -------------------------------------------------------------------
    # Mask miss is multipled with the loss,
...
def process_image(image_rec, img_id, image_index, img_anns, dataset_type):
    ...
    for p in range(numPeople):
        ...
        pers["scale_provided"] = img_anns[p]["bbox"][3] / image_size  # todo: achieve the main person?  main person占比
    ...
    for pers in all_persons:  # 最外层循环是所有的单个的人，从all person中选取main　person
        ...
        for pc in prev_center:  # prev_center 保存了person　center 以及人体框长和宽中的最大值
            ...
            if dist < pc[2] * 0.3:  # pc[2] 代表人体框长和宽中最大的那一边,原始程序中把距离main person特别近，<0.3的不再作为main person
                flag = 1
                continue
...
def writeImage(grp, img_grp, data, img, mask_miss, count, image_id, mask_grp=None):
    ...
    if not img_key in img_grp:
        ...
        if mask_grp is None:
            ...
            # _, img_bin = cv2.imencode(".jpg", img) todo data=img_bin data=img_mask
