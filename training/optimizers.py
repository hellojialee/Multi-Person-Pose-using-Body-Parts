...
# I think a temporary way to do this is to modify your optimizer, i.e. copy the original keras code of optimizers,
# and replace every lr with your own definition.

# For example, using SGD to train the last layer at lr=0.01, the other lr*0.1=0.001:
# First, copy the code from keras.optimizers.SGD and define a new optimizer MultiSGD. Make 2 changes:
#
# In __init__, add a list exception_vars and a multiplier=0.1 to the arguments. Variables in the list will not be
# applied the multiplier.
# In get_updates(), add a new line at the beginning of the loop:
# multiplied_lr = lr * self.multiplier if p in self.exception_vars else lr.

# Then, in each line where lr is used to calculate updates,
# i.e. v = self.momentum * m - lr * g and new_p = p + self.momentum * v - lr * g,
# replace lr with multiplied_lr.
...
# You can modify other optimizers or apply more complicated multipliers in similar ways.
# I am not sure if this is 100% correct, but it works perfectly on my computer.
...
class MultiAdam(Optimizer):
    ...
    def __init__(self, lr=0.001, beta_1=0.9, beta_2=0.999,
                 epsilon=None, decay=0., amsgrad=False, lr_mult=None, **kwargs):
    ...
    def get_updates(self, loss, params):
        ...
        for p, g, m, v, vhat in zip(params, grads, ms, vs, vhats):
            ...
            if p.name in self.lr_mult:
                multiplied_lr = lr_t * self.lr_mult[p.name]
            else:
                multiplied_lr = lr_t

            if self.amsgrad:
                vhat_t = K.maximum(vhat, v_t)
                p_t = p - multiplied_lr * m_t / (K.sqrt(vhat_t) + self.epsilon)
                self.updates.append(K.update(vhat, vhat_t))
            else:
                p_t = p - multiplied_lr * m_t / (K.sqrt(v_t) + self.epsilon)
